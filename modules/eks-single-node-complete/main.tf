...
data "aws_iam_policy_document" "ekscluster-role" {
  source_json = file(var.ekscluster-role-json)
}
...
data "aws_iam_policy_document" "eksworkernode-role" {
  source_json = file(var.workernode-role-json)
}
...
data "aws_iam_policy_document" "workernode-albpolicy" {
  source_json = file(var.workernode-albpolicy)
}
...
data "aws_iam_policy_document" "workernode-albpolicy2" {
  source_json = file(var.workernode-alb-additionalpolicy)
}
...
resource "aws_eks_node_group" "node" {
  ...
  node_group_name = "${each.value.name}-worker-nodes"
  node_role_arn   = aws_iam_role.eks_nodes.arn
  subnet_ids      = var.eks-node-subnets
  instance_types  = var.eks-instancetypes
  ami_type        = var.eks-amitype
  capacity_type   = var.eks-capacitytype
  disk_size       = var.eks-disksize
  ...
  scaling_config {
    desired_size = var.eks-desiredsize
    max_size     = var.eks-maxsize
    min_size     = var.eks-minsize
  }

  labels = {
    Name = "${each.value.name}-worker-node"
  }

  tags = {
    Name = "${each.value.name}-worker-nodes"
    Environment = "${var.env-name}"
  }
...
data "aws_iam_policy_document" "cluster-autoscaler-policy" {
  source_json = file(var.eks-cluster-autoscaler-policy)
}

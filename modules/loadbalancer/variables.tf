...
variable "security_group_ids" {
  type        = list(string)
  description = "security groups for which the ALB will be associated with"
}
...
variable "default_alb_url" {
  type        = string
  description = "Default URL to forward the user if there is no ALB route rules that match, domain"
}
